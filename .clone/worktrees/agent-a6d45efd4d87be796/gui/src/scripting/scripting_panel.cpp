...
void ScriptingPanel::setupUi()
{
    ...
    m_console->setPlaceholderText("Script output will appear here…");
...
void ScriptingPanel::onRun()
{
    ...
    cfg.max_output_bytes = 1 << 20; // 1 MiB
    cfg.on_print = [this](std::string_view text) {
        // Called from the Lua worker thread — queue to GUI thread
    ...
    // Run in a detached thread; notify GUI on completion
    std::thread([this, code]() {
        const auto result = m_engine->run(code);

        // Post result display back to GUI thread
